...
void *write_raster_chunk(void *args) {
    ...
    for (int row = 0; row < wargs->num_rows; row++) {
        ...
        if (GDALRasterIO(band, GF_Write, 0, wargs->start_row + row, wargs->ncols, 1,
                         row_data, wargs->ncols, 1, GDT_Float32, 0, 0) != CE_None) {
...
void write_raster(const char *filename, float *data, int nrows, int ncols, double no_data_value, const char *ref_file) {
    ...
    int num_threads = 6; // optimal for your setup
    int chunk_size = (nrows + num_threads - 1) / num_threads; // ensure exact division
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        args[i].ncols = ncols;
        if (args[i].start_row + args[i].num_rows > nrows) {
            args[i].num_rows = nrows - args[i].start_row;
        }
        printf("writing chunk: start_row=%d, num_rows=%d, offset=%d\n", args[i].start_row, args[i].num_rows, args[i].start_row * ncols);
