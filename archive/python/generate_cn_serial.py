...
def resample_raster(source_path, target_shape, target_transform):
    ...
    dst_ds = mem_driver.Create(
        "", target_shape[1], target_shape[0], 1, gdal.GDT_Float32
    )
...
def save_raster(data, transform, crs, output_path):
    ...
    dst_ds = driver.Create(
        output_path, data.shape[1], data.shape[0], 1, gdal.GDT_Byte, ["COMPRESS=LZW"]
    )
...
def main(block_ids=None, start_block_id=2643):
    ...
    if block_ids:
        blocks_gdf = blocks_gdf[
            blocks_gdf["ID"].isin(block_ids) & (blocks_gdf["ID"] >= start_block_id)
        ]

    # Iterate through the blocks with a progress bar
    for _, block in tqdm(
        blocks_gdf.iterrows(), total=len(blocks_gdf), desc="Processing blocks"
    ):
