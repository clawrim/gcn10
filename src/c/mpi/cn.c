...
load_lookup_table(const char *hc, const char *arc, int table[256][5])
{
	...
	if (snprintf(fname, sizeof(fname), "%s/default_lookup_%s_%s.csv",
		...
		snprintf(msg, sizeof(msg), "lookup table path too long: %s",
			 fname);
	...
	f = fopen(fname, "r");
	if (!f) {
		snprintf(msg, sizeof(msg), "cannot open lookup table %s",
			 fname);
		log_message("ERROR", msg, true);
		MPI_Abort(MPI_COMM_WORLD, 1);
	}
	...
	while (fgets(line, sizeof(line), f)) {
		...
		if (!us) {
			snprintf(msg, sizeof(msg),
				 "invalid grid_code in %s: %s", fname,
				 grid_code);
			log_message("ERROR", msg, true);
			continue;
		}
		*us = '\0';
		lc = atoi(grid_code);
		sg = (us[1] == 'A' ? 1 : us[1] == 'B' ? 2 : us[1] ==
		      'C' ? 3 : 4);
		tok = strtok(NULL, ",");
		if (!tok) {
			snprintf(msg, sizeof(msg),
				 "invalid row in %s: missing cn", fname);
			log_message("ERROR", msg, true);
			continue;
		}
		cnv = atoi(tok);
		if (lc >= 0 && lc < 256 && sg >= 0 && sg < 5) {
			...
			snprintf(msg, sizeof(msg),
				 "invalid values in %s: lc=%d, sg=%d", fname,
				 lc, sg);
			log_message("ERROR", msg, true);
		}
	}
...
static void modify_hysogs_data(uint8_t *h, int npix, const char *cond)
{
	int i;
...
static void
calculate_cn(const uint8_t *esa, const uint8_t *hsg, int npix,
	     int table[256][5], uint8_t *out)
{
	...
	for (i = 0; i < npix; i++) {
		land_cover = esa[i];
		soil_group = hsg[i];
		if (land_cover >= 0 && land_cover < 256 && soil_group >= 0
		    && soil_group < 5) {
...
void process_block(int block_id, bool overwrite, int total_blocks)
{
	...
	if (snprintf(filter, sizeof(filter), "\"ID\"=%d", block_id) >=
	    sizeof(filter)) {
		snprintf(msg, sizeof(msg),
			 "filter string too long for block %d", block_id);
		log_message("ERROR", msg, true);
		OGR_DS_Destroy(ds);
		MPI_Abort(MPI_COMM_WORLD, 1);
	}
	...
	if (!esa) {
		snprintf(msg, sizeof(msg), "esa load failed for block %d",
			 block_id);
		log_message("ERROR", msg, true);
		return;
	}
	...
	hysogs_coarse =
	    load_raster(hysogs_data_path, bbox, &hsx, &hsy, soil_gt, &soil_srs);
	if (!hysogs_coarse) {
		snprintf(msg, sizeof(msg), "hysogs load failed for block %d",
			 block_id);
		log_message("ERROR", msg, true);
		free(esa);
		return;
	...
	if (!hysogs_resampled) {
		snprintf(msg, sizeof(msg),
			 "malloc failed for hysogs resampling, block %d",
			 block_id);
		log_message("ERROR", msg, true);
		free(esa);
		free(hysogs_coarse);
	...
	for (int y = 0; y < esay; y++) {
		...
		for (int x = 0; x < esax; x++) {
			...
			hysogs_resampled[y * esax + x] =
			    hysogs_coarse[cj * hsx + ci];
		}
	}
	free(hysogs_coarse);
	...
	for (c = 0; c < 2; c++) {
		if (snprintf(outdir, PATH_MAX, "cn_rasters_%s", conds[c]) >=
		    PATH_MAX) {
			snprintf(msg, sizeof(msg),
				 "output directory path too long for %s",
				 conds[c]);
			log_message("ERROR", msg, true);
			free(esa);
		...
		for (hi = 0; hi < 3; hi++) {
			for (ai = 0; ai < 3; ai++) {
				...
				if (!hysogs_adjusted) {
					snprintf(msg, sizeof(msg),
						 "malloc failed for hysogs adjusted, block %d",
						 block_id);
					log_message("ERROR", msg, true);
					free(esa);
					free(hysogs_resampled);
				...
				modify_hysogs_data(hysogs_adjusted, npix,
						   conds[c]);
				...
				if (!cn) {
					snprintf(msg, sizeof(msg),
						 "malloc failed for cn raster, block %d",
						 block_id);
					log_message("ERROR", msg, true);
					free(esa);
					free(hysogs_resampled);
				...
				memset(cn, 255, npix);
				calculate_cn(esa, hysogs_adjusted, npix, table,
					     cn);

				/* construct output path */
				len =
				    strlen(outdir) + strlen(hcs[hi]) +
				    strlen(arcs[ai]) + 32;
				outpath = malloc(len);
				if (!outpath) {
					snprintf(msg, sizeof(msg),
						 "malloc failed for output path, block %d",
						 block_id);
					log_message("ERROR", msg, true);
					free(esa);
					free(hysogs_resampled);
				...
				if (snprintf
				    (outpath, len, "%s/cn_%s_%s_%d.tif", outdir,
				     hcs[hi], arcs[ai], block_id) >= len) {
					snprintf(msg, sizeof(msg),
						 "output path too long for block %d",
						 block_id);
					log_message("ERROR", msg, true);
					free(esa);
					free(hysogs_resampled);
				...
				if (!overwrite) {
					...
					if (f) {
						...
						if (!newpath) {
							snprintf(msg,
								 sizeof(msg),
								 "malloc failed for new output path, block %d",
								 block_id);
							log_message("ERROR",
								    msg, true);
							free(esa);
						...
						if (snprintf
						    (newpath, len + 1,
						     "%s/cn_%s_%s_%d_.tif",
						     outdir, hcs[hi], arcs[ai],
						     block_id) >= len + 1) {
							snprintf(msg,
								 sizeof(msg),
								 "new output path too long for block %d",
								 block_id);
							log_message("ERROR",
								    msg, true);
							free(esa);
...
void report_block_completion(int block_id, int total_blocks)
{
	...
	if (rank != 0) {
		...
		blocks_completed++;
		char msg[8192];
		snprintf(msg, sizeof(msg), "progress: %d/%d blocks completed",
			 blocks_completed / 6, total_blocks);
