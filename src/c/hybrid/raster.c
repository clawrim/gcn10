...
int get_all_blocks(int **out_ids)
{
    ...
    while ((feat = OGR_L_GetNextFeature(layer))) {
        ids[i++] =
            OGR_F_GetFieldAsInteger(feat, OGR_F_GetFieldIndex(feat, "ID"));
        OGR_F_Destroy(feat);
    }
    OGR_DS_Destroy(ds);
    *out_ids = ids;
...
uint8_t *load_raster(const char *path,
                     const double *bbox,
                     int *xsize, int *ysize,
                     double *gt, OGRSpatialReferenceH *srs)
{
    register_drivers();
    ...
    *srs = OSRNewSpatialReference(wkt);
    ...
    CPLErr err = GDALRasterIO(GDALGetRasterBand(ds, 1),
                              GF_Read,
...
void save_raster(const uint8_t *data,
                 int xsize,
                 int ysize,
                 const double *gt, OGRSpatialReferenceH srs, const char *path)
{
    register_drivers();
    ...
    GDALDatasetH ds = GDALCreate(drv, path, xsize, ysize, 1, GDT_Byte, opts);
